...
- `STOP_USE_RECENT_EXTREME=true` (place SL at most recent swing extreme: LONG uses recent low, SHORT uses recent high)
- `STOP_RECENT_LOOKBACK_CANDLES=20` (lookback window for swing extreme)
- `STOP_RECENT_BUFFER_PCT=0.00` (optional buffer beyond extreme, in percent)
...
- `STOP_USE_RECENT_EXTREME` (default `true`)
- `STOP_RECENT_LOOKBACK_CANDLES` (default `20`)
