...
- `STOP_USE_NEAREST_PIVOT=true` (anchor SL to nearest recent pivot low/high; falls back to most recent low/high if no pivot)
- `STOP_PIVOT_LOOKBACK_CANDLES=50` (lookback window used to find nearest pivot)
- `STOP_PIVOT_BUFFER_PCT=0.00` (optional buffer beyond pivot, in percent)
...
- `STOP_USE_NEAREST_PIVOT` (default `true`)
- `STOP_PIVOT_LOOKBACK_CANDLES` (default `50`)
- `STOP_PIVOT_BUFFER_PCT` (default `0.00`)
